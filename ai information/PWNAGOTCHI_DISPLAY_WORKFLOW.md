...
For display text, keep it simple and visible. Use pwnagotchi.ui.components.LabeledValue, pwnagotchi.ui.fonts, and BLACK from pwnagotchi.ui.view. Register the element in on_ui_setup(), update it in on_ui_update(), and remove it in on_unload(). On this firmware and layout, position (8, 96) with Small fonts places the text in the blank lower-left band under BT:Trusted and above the bottom divider line. A single line works better than a multi-line block. The confirmed display is: label "HWP", value "SET URL", "READY", or "QUEUED N".
